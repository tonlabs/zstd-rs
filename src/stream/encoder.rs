...
#[derive(PartialEq)]
enum EncoderState {
    /// Default state: input is allowed.
    Accepting,
    /// We're in the process of closing the stream.
    ///
    /// We're not accepting any input.
    Finishing,
}
...
impl<W: Write> Encoder<W> {
    ...
    pub fn with_dictionary(
        ...
        raw::Encoder::with_dictionary(level, dictionary).map(|encoder| {
            Encoder {
                writer: zio::Writer::new(writer, encoder),
...
impl<W: Write> Write for Encoder<W> {
    ...
    fn flush(&mut self) -> io::Result<()> {
        match self.state {
            EncoderState::Accepting => self.writer.flush(),
            EncoderState::Finishing => self.writer.finish(),
        }
    }
}
...
mod tests {
    ...
    fn setup_partial_write(
        ...
        assert_ne!(z.writer.offset(), z.writer.buffer().len());
...
mod async_tests {
    ...
    use partial_io::{GenWouldBlock, PartialAsyncWrite,
                     PartialWithErrors};
